...
const copyImages = async (input, output) => {
  const images = await globby(
    path.resolve(input, '**', '*.{gif,png,jpg,jpeg,svg,svgz}'),
  );

  return Promise.all(
    images.map(async (image) => {
      const outputFile = path.resolve(output, path.relative(input, image));
      await fsExtra.copy(image, outputFile);
      console.debug(`write: ${outputFile}`);
    }),
  );
};
